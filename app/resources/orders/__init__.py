...
class OrdersAPI(ResourceAPI):
    ...
    def post(self) -> Response:
        ...
        if request_json is None:
            return make_error_request_response(HTTPStatus.BAD_REQUEST, error=BillingError.REQUEST_NO_JSON)
        ...
        modify_date = request_json.get(OrderDB._modify_date_field, None)
        modify_reason = request_json.get(OrderDB._modify_reason_field, None)

        order_db = OrderDB(storage_service=self.__db_storage_service, code=code, status_id=status_id,
                           payment_uuid=payment_uuid, modify_date=modify_date, modify_reason=modify_reason)
    ...
    def put(self, suuid: str) -> Response:
        ...
        if not is_valid_suuid or not is_valid_order_uuid or suuid != order_uuid:
            return make_error_request_response(HTTPStatus.BAD_REQUEST, error=BillingError.ORDER_IDENTIFIER_ERROR)
    ...
    def get(self, suuid: str = None) -> Response:
        ...
        order_db = OrderDB(storage_service=self.__db_storage_service, suuid=suuid, limit=self.pagination.limit,
